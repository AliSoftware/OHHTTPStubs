...
## [4.0.0](https://github.com/AliSoftware/OHHTTPStubs/releases/tag/4.0.0)

* Annotated the library with nullability attributes for ease of use in Swift
* Migrated the path utility macros to `OHPathHelpers.h` and converted them to functions for Swift support ([#100](https://github.com/AliSoftware/OHHTTPStubs/issues/100)).
* Added a complete Swift Example ([#88](https://github.com/AliSoftware/OHHTTPStubs/issues/88))

> **Note**: This vis a major version bump (`4.0.0` and not `3.2.0`) because the added nullability attributes changes the Swift API (from optionals to non-optionals for some parameters), but there is no change in API for Objective-C users.
